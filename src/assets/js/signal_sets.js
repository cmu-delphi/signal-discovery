...
function format (signalSetId, relatedSignals, signalSetDescription) {
    ...
    if (signals.length > 0) {
        ...
        signals.forEach((signal) => {
            ...
            var disabled = signal.endpoint ? "" : "disabled title='Visualization functionality for this endpoint is coming soon'";
            tableMarkup += '<tr>'+
                                `<td><input ${disabled} type="checkbox" name="selectedSignal" onclick="addSelectedSignal(this)" data-signal-displayname='${signal.display_name}' data-endpoint="${signal.endpoint}" data-datasource="${signal.source}" data-signal="${signal.name}" data-time-type="${signal.time_type}" data-signal-set="${signal.signal_set_name}" data-signal-set-short-name="${signal.signal_set_short_name}" data-member-short-name="${signal.member_short_name}" ${checked}></td>`+
